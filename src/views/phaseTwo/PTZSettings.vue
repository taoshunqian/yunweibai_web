  ...
  >
    ...
    <Cell>
      <template #title>
        <ul>
          <li>
            波特率: <span>{{ item[1] }}</span>
          </li>
          <li>
            数据位: <span>{{ item[2] + " 位" }}</span>
          </li>
          <li>
            停止位: <span>{{ item[3] + " 位" }}</span>
          </li>
        </ul>
      </template>
      <template #right-icon>
        <ul>
          <li>
            校验位: <span>{{ columnsCheckbits[item[4]] }}</span>
          </li>
          <li>
            协议类型: <span>{{ columnAgreementType[item[5]] }}</span>
          </li>
          <li>
            解码器地址: <span>{{ item[6] }}</span>
          </li>
        </ul>
      </template>
    </Cell>
